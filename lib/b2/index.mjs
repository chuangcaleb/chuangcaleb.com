...
const downloadFile = async (fileName, auth) => {
	try {
		...
		if (pathWithoutPrefix === '/notes.zip') {
			const destPath = path.resolve(B2_DEST_DIR);

			console.log('🚀 ~ downloadFile ~ B2_DEST_DIR:', B2_DEST_DIR);
			console.log('🚀 ~ downloadFile ~ destPath:', destPath);

			await fs.mkdir(path.dirname(destPath), {recursive: true});
			await extractZipStreamToDir(response.body, destPath);
			return;
		}
...
function logExtractedFiles(dir, prefix = '') {
	for (const file of readdirSync(dir)) {
		const fullPath = path.join(dir, file);
		const stat = statSync(fullPath);
		if (stat.isDirectory()) {
			logExtractedFiles(fullPath, prefix + file + '/');
		} else {
			console.log(`✅ Extracted: ${prefix + file}`);
		}
	}
}
