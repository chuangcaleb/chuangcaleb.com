...
const downloadFile = async (fileName, auth) => {
	try {
		...
		if (pathWithoutPrefix === '/notes.zip') {
			...
			await fs.rm(B2_DEST_DIR, {recursive: true, force: true});
			await fs.mkdir(B2_DEST_DIR, {recursive: true});
			await decompress(buffer, B2_DEST_DIR, {strip: 1});
			return;
		}
...
async function main() {
	try {
		...
		await Promise.all(files.map(file => downloadFile(file.fileName, authData)));
		console.log('✅ All files downloaded successfully.');
	} catch (error) {
		throw new Error(`❌ B2 sync failed: ${error.message}`, {cause: error});
	}
}
