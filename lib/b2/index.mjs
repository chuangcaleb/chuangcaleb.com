...
function removeNotesPrefix(path) {
	return path.startsWith(B2_BUCKET_PREFIX)
		? path.slice(B2_BUCKET_PREFIX.length)
		: path;
}
...
const downloadFile = async (file) => {
	try {
		...
		const filePath = path.join(B2_DEST_DIR, removeNotesPrefix(file.fileName));
